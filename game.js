...
function renderSnake() {
	if (over) gameOver();
		...
		for (let i = 0; i < snake.position.length; i++) {
			boxes[
				snake.position[i][0] + snake.position[i][1] * table.rowsColumns
			].classList.add("snake");
		}
	}
}
...
function checkColissionWithFood() {
	...
	for (let i = 0; i < snake.position.length - 1; i++) {
		if (
			boxes[snake.position[i][0] + snake.position[i][1] * table.rowsColumns] ===
			boxes[head[0] + head[1] * table.rowsColumns]
		) {
			over = true;
			gameOver();
		}
...
function hitBorder() {
	...
	// goes of limits
	if (
		(snake.position[headPos][0] === table.rowsColumns - 1 &&
			snake.direction === "right") ||
		(snake.position[headPos][0] === 0 && snake.direction === "left") ||
		(snake.position[headPos][1] === table.rowsColumns - 1 &&
...
function updatePostion() {
	boxes[
		snake.position[0][0] + snake.position[0][1] * table.rowsColumns
